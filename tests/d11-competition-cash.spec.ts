...
test('purchasing a competition level deducts echoes and advances the level', async ({ page }) => {
  await page.goto('/?seed=1&echoes=30');
...
test('competition cannot level past the max', async ({ page }) => {
  await page.goto('/?seed=1&competitionLevel=1&echoes=100');
  await page.getByTestId('competition-level-up').click();
...
test('prestige resets competition level and cash', async ({ page }) => {
  await page.goto('/?seed=1&competitionLevel=2&cash=50&loop=50');
